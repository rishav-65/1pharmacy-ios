...
const cardPropParser = (item: any) => ({
    ...
    highlightDetails: [
        `T.Balance: ₹ ${(item.dueAmount || 0).toLocaleString('en-US', { maximumFractionDigits: 2 })}`
    ],
    actions: [
        {
            action: 'link',
...
const Customers = ({ isFocused, bottomTabsMounted }: { isFocused: boolean, bottomTabsMounted?: boolean }) => {
    ...
    const fetchRenderData = ({ onCompleteCallback, isRefreshing }: { onCompleteCallback?: Function, isRefreshing?: boolean }) => {
        ...
        APIGet({
            ...
            resolve: (response: any) => {
                ...
                const itemsWithCardID = (response.data?.customers || []).map((item: any) => ({ ...item, card_id: 'card_10' }));
        ...
        <View>
            {
                loading
                    ? <LoadingScreen />
                    : <ListView
                        title={`Customers (${(inventoryData.items || []).length} customers)`}
